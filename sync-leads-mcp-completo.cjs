 ...
 * 
 * EXECUTAR NO SERVIDOR: node sync-leads-mcp-completo.cjs
 */
...
// Importar fetch se não estiver disponível (Node 18+)
...
// Função para mapear lead (mesma do api-sync-opportunities.js)
...
// Função para gerar SQL de upsert em lote
function generateUpsertSQL(leads, batchNumber) {
    ...
    const updateFields = fields.filter(f => f !== 'id').map(field => {
        return `"${field}" = EXCLUDED."${field}"`;
    }).join(',\n                ');

    return `
-- Lote ${batchNumber}: ${leads.length} leads
INSERT INTO api.leads (${fields.map(f => `"${f}"`).join(', ')})
VALUES 
        ${values}
...
async function main() {
    console.log('🚀 Iniciando sincronização completa de leads via MCP...\n');
    ...
    // FASE 2: Gerar SQL para criar tabela temporária
    const sprintHubIdsArray = Array.from(sprintHubIds);
    const createTempTableSQL = `
-- Criar tabela temporária com IDs do SprintHub
CREATE TEMP TABLE IF NOT EXISTS temp_sprinthub_lead_ids (
    id BIGINT PRIMARY KEY
);

-- Inserir IDs (primeiros 1000 para exemplo)
INSERT INTO temp_sprinthub_lead_ids (id)
VALUES ${sprintHubIdsArray.slice(0, 1000).map(id => `(${id})`).join(',\n       ')}
ON CONFLICT (id) DO NOTHING;
`;

    // FASE 3: SQL para deletar leads que não existem mais
    const deleteSQL = `
...
`;
    ...
    console.log('📋 SQLs gerados para execução via MCP:\n');
    console.log('1. Criar tabela temporária e inserir IDs:');
    console.log(createTempTableSQL.substring(0, 500) + '...\n');
    console.log('2. Deletar leads ausentes:');
    console.log(deleteSQL);

    // FASE 4: Buscar todos os leads e gerar SQLs de upsert
    console.log('\n📥 FASE 4: Buscando todos os leads com dados completos...');
    page = 0;
    let allLeads = [];
    let batchNumber = 1;
    ...
    while (true) {
        ...
        console.log(`   ✅ Página ${page + 1}: ${mapped.length} leads mapeados (Total: ${allLeads.length})`);
        
        page++;
        await sleep(DELAY_MS);
    }

    console.log(`\n✅ Total de leads mapeados: ${allLeads.length}\n`);

    // Gerar SQLs de upsert em lotes
    console.log('📝 Gerando SQLs de upsert em lotes...\n');
    const sqlBatches = [];
    for (let i = 0; i < allLeads.length; i += BATCH_SIZE) {
        const batch = allLeads.slice(i, i + BATCH_SIZE);
        const sql = generateUpsertSQL(batch, Math.floor(i / BATCH_SIZE) + 1);
        if (sql) {
            sqlBatches.push(sql);
        }
    }

    console.log(`✅ ${sqlBatches.length} lotes de SQL gerados (${BATCH_SIZE} leads por lote)\n`);
    console.log('💡 Execute os SQLs via MCP na ordem:\n');
    console.log('   1. Criar tabela temporária com IDs');
    console.log('   2. Deletar leads ausentes');
    console.log('   3. Executar lotes de upsert (um por vez)\n');

    return {
        sprintHubTotal: sprintHubIds.size,
        totalLeads: allLeads.length,
        sqlBatches: sqlBatches.length,
        createTempTableSQL,
        deleteSQL,
        firstBatchSQL: sqlBatches[0]?.substring(0, 500) + '...'
    };
}
...
if (require.main === module) {
    main().then(result => {
        console.log('\n📊 Resumo:');
        console.log(`   Total SprintHub: ${result.sprintHubTotal}`);
        console.log(`   Total leads mapeados: ${result.totalLeads}`);
        console.log(`   Lotes SQL gerados: ${result.sqlBatches}`);
