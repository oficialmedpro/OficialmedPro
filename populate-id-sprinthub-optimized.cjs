...
// Função principal
async function populateIdSprinthub() {
    console.log('🚀 POPULANDO ID_SPRINTHUB COM MATCH OTIMIZADO...\n');
    
    try {
        // 1. Buscar leads exportados com dados válidos
        console.log('🔍 Buscando leads exportados com dados válidos...');
        ...
        const { data: exportedLeads, error: exportedError } = await supabase
            ...
            .neq('primeiro_nome', '')
            .limit(500); // Processar 500 para teste
        ...
        // 2. Buscar dados da API do SprintHub (múltiplas páginas)
        console.log('\n🔄 Buscando dados da API do SprintHub...');
        
        let apiLeads = [];
        let page = 0;
        const PAGE_LIMIT = 100;
        const MAX_PAGES = 20; // Buscar até 2000 leads da API
        ...
        while (page < MAX_PAGES) {
            ...
            console.log(`📄 Página ${page + 1}: ${leadsPage.length} leads da API`);
            page++;
        ...
        // 3. Fazer match e inserir/atualizar
        ...
        const { count: validCount } = await supabase.from('leads').select('*', { count: 'exact' }).not('firstname', 'is', null).neq('firstname', '');
        ...
        // 6. Amostra dos leads com ID do SprintHub
