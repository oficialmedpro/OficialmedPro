...
async function getGoogleAdsCredentials() {
  try {
    ...
    // Primeiro tentar buscar todas as unidades
    const response = await fetch(`${supabaseUrl}/rest/v1/unidades?select=*`, {
      method: 'GET',
      headers: {
    ...
    // Buscar a unidade Apucarana
    const unidade = data.find((u: any) => 
      u.unidade === 'APUCARANA' || 
      u.unidade === 'Apucarana' ||
      (u.codigo_sprint && Array.isArray(u.codigo_sprint) && u.codigo_sprint.includes(1)) ||
      (u.codigo_sprint && u.codigo_sprint === '[1]') ||
      (u.codigo_sprint && u.codigo_sprint.toString().includes('1'))
    )

    if (!unidade) {
      console.log('❌ Unidades disponíveis:', data.map((u: any) => ({ nome: u.unidade, codigo: u.codigo_sprint })))
      throw new Error('Unidade Apucarana não encontrada')
    }

    console.log('✅ Unidade encontrada:', unidade.unidade)
