...
-- ============================================================================
-- Este script modifica a função de consolidação automática para respeitar
-- os campos que foram editados manualmente através da interface
-- ============================================================================

-- IMPORTANTE: Execute primeiro o script create-campos-protegidos-system.sql
...
BEGIN
  ...
  IF FOUND THEN
    ...
    UPDATE api.clientes_mestre SET
      ...
      -- MAS SEMPRE RESPEITANDO CAMPOS PROTEGIDOS
  ...
  ELSE
    -- INSERIR novo cliente (sem necessidade de verificar proteção, é novo)
...
'Consolida dados de clientes de múltiplas fontes RESPEITANDO campos protegidos.
Campos editados manualmente através da interface não serão sobrescritos pela consolidação automática.
Versão atualizada para respeitar api.campos_protegidos.';

-- Mensagem de sucesso
DO $$
BEGIN
  RAISE NOTICE '✅ Função de consolidação atualizada com sucesso!';
  RAISE NOTICE '🛡️ Agora os triggers respeitam campos editados manualmente';
  RAISE NOTICE '📝 Campos protegidos não serão sobrescritos pela consolidação automática';
  RAISE NOTICE '';
  RAISE NOTICE '💡 Para visualizar campos protegidos:';
  RAISE NOTICE '   SELECT * FROM api.vw_campos_protegidos_auditoria;';
  RAISE NOTICE '';
  RAISE NOTICE '💡 Para desproteger um campo:';
  RAISE NOTICE '   SELECT api.desproteger_campo(cliente_id, ''nome_campo'');';
END $$;
