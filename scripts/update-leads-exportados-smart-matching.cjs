...
// Função para normalizar telefone APENAS para comparação
...
async function updateLeadsExportadosWithSmartMatching() {
    console.log('🔄 ATUALIZANDO leads_exportados_sprinthub com matching inteligente...');
    ...
    try {
        ...
        // 2. Criar mapa de lead_id usando telefones normalizados para comparação
        console.log('\n🔗 Criando mapa com telefones normalizados para comparação...');
        ...
        allOportunidades.forEach(oportunidade => {
            ...
            if (whatsapp && whatsapp.trim() !== '') {
                const normalizedPhone = normalizePhoneForComparison(whatsapp);
                if (normalizedPhone) {
        ...
        // 4. Processar matches usando normalização APENAS para comparação
        console.log('\n🔗 Processando matches com normalização inteligente...');
        ...
        for (let i = 0; i < allExportedLeads.length; i += UPDATE_BATCH_SIZE) {
            ...
            for (const exportedLead of batch) {
                ...
                // Tentar match por telefone normalizado (APENAS para comparação)
                if (exportedLead.whatsapp && exportedLead.whatsapp.trim() !== '') {
                    const normalizedPhone = normalizePhoneForComparison(exportedLead.whatsapp);
                    if (normalizedPhone) {
                ...
                if (leadId) {
                    ...
                    // Atualizar APENAS o campo id_sprint com o lead_id encontrado
        ...
        // 5. Relatório final
        console.log('\n🎉 ATUALIZAÇÃO CONCLUÍDA COM MATCHING INTELIGENTE!');
...
updateLeadsExportadosWithSmartMatching();
