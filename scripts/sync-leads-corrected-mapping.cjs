    ...
    try {
        ...
        if (response.status === 401) {
            ...
            if (errorData.msg.includes('too many requests')) {
                console.log(`⏳ Rate limit atingido. Aguardando ${DELAY_BETWEEN_PAGES / 1000} segundos antes de tentar novamente...`);
...
async function syncLeads() {
    console.log('🚀 Iniciando sincronização de leads (versão corrigida com mapeamento correto)...');
    let page = 0;
    ...
    while (true) {
        ...
        console.log(`✅ Página ${page + 1}: ${totalInserted - (page * PAGE_LIMIT)} inseridos, ${totalErrors - (page * PAGE_LIMIT)} erros`);
