...
const CONFIG = {
  SPRINTHUB: {
    ...
    PAGE_LIMIT: 200,
    DELAY_BETWEEN_PAGES: 60000 // 60 segundos entre páginas (respeitando rate limit)
  },
  SUPABASE: {
...
async function waitForRateLimit() {
  log('⏳ Aguardando rate limit resetar (60 segundos)...');
  await new Promise(resolve => setTimeout(resolve, 60000));
  log('✅ Rate limit resetado, continuando...');
}

// Função para fazer requisição com retry
async function fetchSprinthubData(url, retries = 3) {
    ...
    try {
      ...
      if (response.status === 401) {
        ...
        if (errorData.msg && errorData.msg.includes('too many requests')) {
          log(`⚠️ Rate limit atingido. Aguardando...`);
...
// Função principal
async function syncLeadsWithRateLimit() {
  log('🚀 Iniciando sincronização com controle de rate limit...');
  ...
  while (hasMorePages) {
    try {
      ...
      // Delay entre páginas (respeitando rate limit)
      if (hasMorePages) {
...
syncLeadsWithRateLimit().catch(error => {
