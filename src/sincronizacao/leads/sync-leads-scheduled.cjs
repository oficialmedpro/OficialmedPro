...
// Função principal
async function syncLeadsScheduled() {
  log('🚀 Iniciando sincronização agendada...');
...
// Aguardar 5 minutos antes de executar
async function main() {
  log('⏰ Aguardando 5 minutos antes de iniciar a sincronização...');
  log('🕐 Início: ' + new Date().toLocaleString('pt-BR'));
  
  // Aguardar 5 minutos (300 segundos)
  await new Promise(resolve => setTimeout(resolve, 300000));
  
  log('🚀 Iniciando sincronização após 5 minutos de espera...');
  await syncLeadsScheduled();
}
