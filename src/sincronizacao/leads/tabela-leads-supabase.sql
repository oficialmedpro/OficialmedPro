...
CREATE INDEX idx_leads_fullname ON api.leads(fullname);
CREATE INDEX idx_leads_created_at ON api.leads(created_at);
...
COMMENT ON COLUMN api.leads.fullname IS 'Nome completo do lead';
...
COMMENT ON COLUMN api.leads.archived IS 'Flag para indicar se está arquivado (0=ativo, 1=arquivado)';

-- 🛡️ RLS (Row Level Security) - Opcional
-- ALTER TABLE api.leads ENABLE ROW LEVEL SECURITY;
-- CREATE POLICY "Leads são visíveis para usuários autenticados" ON api.leads FOR SELECT USING (auth.role() = 'authenticated');
