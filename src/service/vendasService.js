/**
 * 📊 SERVIÇO PARA DADOS DO VENDAS PWA
 * 
...
export const getKpisAcolhimento = async (filtros = {}) => {
  try {
    ...
    // Agregar todos os vendedores se não filtrar por vendedor específico
    ...
    // Retornar dados do primeiro vendedor se não houver agregação
    return data?.[0] || {};
  } catch (error) {
...
/**
 * Busca KPIs de Orçamentista
 * @param {Object} filtros - { unidadeId, funilId, vendedorId }
 * @returns {Promise<Object>} KPIs agregados
 */
export const getKpisOrcamento = async (filtros = {}) => {
  try {
    console.log('🔍 [vendasService] Buscando KPIs de Orçamento:', filtros);
    ...
    console.log(`✅ [vendasService] KPIs Orçamento encontrados:`, data?.length || 0);
    ...
    console.error('❌ [vendasService] Erro ao buscar KPIs Orçamento:', error);
    throw error;
  }
};
 ...
 * @param {String} aba - 'acolhimento', 'orçamento', 'vendas'
