...
SELECT cron.unschedule('process-segmentos-lote');

-- 2. Criar novo cron job que chama o endpoint do BETA
-- Este endpoint processa 100 leads por vez com delay de 60s entre segmentos
SELECT cron.schedule(
  'process-segmentos-lote-beta', -- Nome do job
  '0 8,10,12,14,16,18,20 * * *', -- Executa às 8h, 10h, 12h, 14h, 16h, 18h, 20h
  $$
  SELECT net.http_post(
    'https://beta.oficialmed.com.br/api/processar-segmentos-lote',
...
-- 4. Testar o endpoint manualmente (opcional)
-- SELECT net.http_post(
--   'https://beta.oficialmed.com.br/api/processar-segmentos-lote',
--   headers => '{"Content-Type": "application/json", "x-auth-token": "meu-token-super-secreto-2024"}'::jsonb,
--   body => '{}'::jsonb
