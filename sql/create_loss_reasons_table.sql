...
CREATE INDEX IF NOT EXISTS idx_loss_reasons_funil_id ON loss_reasons(funil_id);
CREATE INDEX IF NOT EXISTS idx_loss_reasons_name ON loss_reasons(name);

-- Adicionar comentários na tabela
COMMENT ON TABLE loss_reasons IS 'Tabela de motivos de perda de oportunidades organizados por funil';
COMMENT ON COLUMN loss_reasons.id IS 'ID único do motivo de perda (relaciona com oportunidade_sprint.loss_reason)';
COMMENT ON COLUMN loss_reasons.name IS 'Nome do motivo de perda';
COMMENT ON COLUMN loss_reasons.funil_id IS 'ID do funil ao qual o motivo pertence';
COMMENT ON COLUMN loss_reasons.description IS 'Descrição detalhada do motivo de perda';
...
-- Mostrar todos os motivos inseridos
SELECT 
    id,
    name,
    funil_id,
