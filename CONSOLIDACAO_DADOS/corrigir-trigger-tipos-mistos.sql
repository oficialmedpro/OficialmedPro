-- ============================================================================
-- CORREÇÃO DEFINITIVA: Lidar com UUID e BIGINT
-- ============================================================================
-- Descrição: Corrige erro "cannot cast type uuid to bigint"
-- Solução: Armazenar ID como TEXT no JSONB e converter no momento certo
-- ============================================================================
...
DECLARE
  ...
  v_id_value BIGINT; -- Variável para armazenar o ID convertido
BEGIN
  ...
  CASE TG_TABLE_NAME
    WHEN 'leads' THEN
      ...
      v_tag := 'sprinthub';
      v_id_value := NEW.id::BIGINT; -- Converter para BIGINT
    ...
    WHEN 'greatpage_leads' THEN
      ...
      -- GreatPage pode ter UUID ou BIGINT, tentar converter
      BEGIN
        v_id_value := NEW.id::TEXT::BIGINT;
      EXCEPTION WHEN OTHERS THEN
        -- Se falhar, usar um hash do UUID
        v_id_value := ('x' || substring(NEW.id::TEXT, 1, 15))::bit(60)::BIGINT;
      END;
    ...
    WHEN 'blacklabs' THEN
      ...
      v_tag := 'blacklabs';
      v_id_value := NEW.id::BIGINT; -- BlackLabs é BIGINT
    ...
    WHEN 'prime_clientes' THEN
      ...
      v_tag := 'prime';
      v_id_value := NEW.id::BIGINT; -- Prime é BIGINT
  ...
  IF FOUND THEN
    -- ATUALIZAR cliente existente usando v_id_value já convertido
    UPDATE api.clientes_mestre SET
      ...
      -- ⭐ Usar v_id_value que já está em BIGINT
      id_sprinthub = CASE WHEN v_fk_column = 'id_sprinthub' THEN v_id_value ELSE id_sprinthub END,
      id_greatpage = CASE WHEN v_fk_column = 'id_greatpage' THEN v_id_value ELSE id_greatpage END,
      id_blacklabs = CASE WHEN v_fk_column = 'id_blacklabs' THEN v_id_value ELSE id_blacklabs END,
      id_prime = CASE WHEN v_fk_column = 'id_prime' THEN v_id_value ELSE id_prime END,
  ...
  ELSE
    ...
    INSERT INTO api.clientes_mestre (
      ...
      v_chave,
      ARRAY[v_tag]::TEXT[],
      CASE WHEN v_fk_column = 'id_sprinthub' THEN v_id_value END,
      CASE WHEN v_fk_column = 'id_greatpage' THEN v_id_value END,
      CASE WHEN v_fk_column = 'id_blacklabs' THEN v_id_value END,
      CASE WHEN v_fk_column = 'id_prime' THEN v_id_value END,
...
SELECT '✅ Função corrigida com tratamento de UUID e BIGINT!' AS status;
