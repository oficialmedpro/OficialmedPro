-- ============================================================================
-- TRIGGERS PARA CONSOLIDAÇÃO AUTOMÁTICA E CONTÍNUA
-- ============================================================================
-- Descrição: Atualiza clientes_mestre automaticamente quando dados mudam
-- Execução: Sempre que há INSERT/UPDATE nas tabelas de origem
-- ============================================================================

-- ============================================================================
-- FUNÇÃO DE CONSOLIDAÇÃO AUTOMÁTICA
-- ============================================================================
...
BEGIN
  ...
  IF FOUND THEN
    ...
    UPDATE api.clientes_mestre SET
      ...
      -- Atualizar foreign key (com cast para BIGINT)
...
-- ============================================================================
-- CRIAR TRIGGERS NAS TABELAS DE ORIGEM
-- ============================================================================

-- Trigger para LEADS (SprintHub)
DROP TRIGGER IF EXISTS trigger_consolidar_leads ON api.leads;
CREATE TRIGGER trigger_consolidar_leads
AFTER INSERT OR UPDATE ON api.leads
FOR EACH ROW
EXECUTE FUNCTION api.consolidar_cliente_automatico();

-- Trigger para GREATPAGE_LEADS
DROP TRIGGER IF EXISTS trigger_consolidar_greatpage ON api.greatpage_leads;
CREATE TRIGGER trigger_consolidar_greatpage
AFTER INSERT OR UPDATE ON api.greatpage_leads
FOR EACH ROW
EXECUTE FUNCTION api.consolidar_cliente_automatico();

-- Trigger para BLACKLABS
DROP TRIGGER IF EXISTS trigger_consolidar_blacklabs ON api.blacklabs;
CREATE TRIGGER trigger_consolidar_blacklabs
AFTER INSERT OR UPDATE ON api.blacklabs
FOR EACH ROW
EXECUTE FUNCTION api.consolidar_cliente_automatico();

-- Trigger para PRIME_CLIENTES
DROP TRIGGER IF EXISTS trigger_consolidar_prime ON api.prime_clientes;
CREATE TRIGGER trigger_consolidar_prime
AFTER INSERT OR UPDATE ON api.prime_clientes
FOR EACH ROW
EXECUTE FUNCTION api.consolidar_cliente_automatico();
...
DO $$
BEGIN
  RAISE NOTICE '✅ Triggers de consolidação automática criados!';
  RAISE NOTICE '';
  RAISE NOTICE 'Triggers ativos:';
  RAISE NOTICE '  - trigger_consolidar_leads (api.leads)';
  RAISE NOTICE '  - trigger_consolidar_greatpage (api.greatpage_leads)';
  RAISE NOTICE '  - trigger_consolidar_blacklabs (api.blacklabs)';
  RAISE NOTICE '  - trigger_consolidar_prime (api.prime_clientes)';
  RAISE NOTICE '';
  RAISE NOTICE 'A partir de agora, qualquer INSERT ou UPDATE nessas tabelas';
  RAISE NOTICE 'irá consolidar automaticamente em clientes_mestre!';
END $$;

-- ============================================================================
-- TESTAR TRIGGER (OPCIONAL)
-- ============================================================================

-- Exemplo de teste:
-- UPDATE api.leads SET email = 'teste@exemplo.com' WHERE id = 1;
-- SELECT * FROM api.clientes_mestre WHERE id_sprinthub = 1;
