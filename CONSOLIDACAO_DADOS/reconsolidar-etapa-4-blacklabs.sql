-- ============================================================================
-- RECONSOLIDAÇÃO INCREMENTAL - ETAPA 4: BLACKLABS
-- ============================================================================
...
DECLARE
  ...
  v_total INTEGER;
BEGIN
  SELECT COUNT(*) INTO v_total FROM api.blacklabs;
  
  RAISE NOTICE '🟠 RECONSOLIDANDO BLACKLABS...';
  RAISE NOTICE '   Total: % registros', v_total;
  RAISE NOTICE '';

  LOOP
    UPDATE api.blacklabs
    SET created_at = NOW()  -- Força atualização real
    WHERE id IN (
      SELECT id FROM api.blacklabs
      ORDER BY id
